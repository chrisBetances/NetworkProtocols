...
def tcp_send(server_host, server_port):
    ...
    while num_lines != 0:
        ...
        time.sleep(15)  # Just to mess with your servers. :-)
...
def tcp_receive(listen_port):
    ...
    listen_socket.listen(5)  # Num of conn. to accept
    data_socket, sender_address = listen_socket.accept()
    print('tcp_receive (server): listen_port={0}'.format(listen_port))
    while verification == b'A':
        verification = read_msg(data_socket, file_tracker)
        data_socket.send(verification)
        file_tracker += 1
        if verification == b'Q':
            data_socket.close()
            listen_socket.close()
...
def get_length(data_socket):
    ...
    """
    length_bytes = next_byte(data_socket)
    for i in range(0, 3):
        length_bytes = length_bytes + next_byte(data_socket)
    line_length = int.from_bytes(length_bytes, 'big')
    return line_length
...
def read_line(data_socket):
    ...
    while b'\n' not in b:
        b = b + next_byte(data_socket)
    return b.decode('ASCII')


def read_msg(data_socket, file_tracker):
    """
    This method converts a message in Hexadecimal to a human readable language

    :param int data_socket: The socket to read from
    :param int file_tracker: incrementer file names
    ...
    if total_length == 0:
        ...
        print(message)
        file_handle = open(str(file_tracker)+'.txt', 'w')
